...
from src.analytics.anomalies import AnomalyDetector, AnomalyType
...
class AnomalyDetail(BaseModel):
    ...
    risk_score: int
    country: Optional[str] = None
    previous_countries: Optional[List[str]] = None
...
async def get_login_analytics(
    ...
    start_time: Optional[datetime] = Query(None, description="Filter from this time (ISO 8601)"),
    end_time: Optional[datetime] = Query(None, description="Filter until this time (ISO 8601)"),
    ...
    country_code: Optional[str] = Query(None, description="Filter by country code (2-letter)"),
    status: Optional[str] = Query(None, description="Filter by status: 'success' or 'failed'"),
    has_anomaly: Optional[bool] = Query(None, description="Filter for logins with anomalies"),
