...
class LoginAnalyticsModel(Base):
    ...
    anomaly_flags: Mapped[list[str]] = mapped_column(
        ...
        default=list,
        nullable=False,
        comment="List of detected anomaly types"
    )
...
class UserLoginHistoryModel(Base):
    ...
    failed_attempts_24h: Mapped[int] = mapped_column(
        default=0,
        nullable=False,
        comment="Failed login attempts in last 24 hours"
    )
...
class AnomalyDetectionConfig(Base):
    ...
    tenant_id: Mapped[str] = mapped_column(
        ...
        ForeignKey("tenants.id"),
        primary_key=True,
        comment="Internal tenant UUID (PK)"
    )
    enabled: Mapped[bool] = mapped_column(
        Boolean,
        default=True,
        nullable=False,
        comment="Whether anomaly detection is enabled"
    )
    impossible_travel_enabled: Mapped[bool] = mapped_column(
        Boolean,
        default=True,
        nullable=False,
        comment="Enable impossible travel detection"
    )
    impossible_travel_speed_kmh: Mapped[int] = mapped_column(
        default=900,
        nullable=False,
        comment="Speed threshold for impossible travel (km/h)"
    )
    new_country_enabled: Mapped[bool] = mapped_column(
        Boolean,
        default=True,
        nullable=False,
        comment="Enable new country detection"
    )
    ...
    risk_score_threshold: Mapped[int] = mapped_column(
        default=70,
        nullable=False,
        comment="Risk score threshold for alerting"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime,
        default=utc_now,
