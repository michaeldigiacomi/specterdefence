...
class MailboxRuleModel(Base):
    ...
    rule_type: Mapped[RuleType] = mapped_column(
        SQLEnum(RuleType, native_enum=False, values_callable=lambda x: [e.value for e in x]), nullable=False, comment="Type of mailbox rule"
    )
    is_enabled: Mapped[bool] = mapped_column(
        Boolean, default=True, nullable=False, comment="Whether the rule is enabled in Exchange"
    )
    status: Mapped[RuleStatus] = mapped_column(
        SQLEnum(RuleStatus, native_enum=False, values_callable=lambda x: [e.value for e in x]),
        nullable=False,
        default=RuleStatus.ACTIVE,
        comment="Analysis status of the rule",
    )
    severity: Mapped[RuleSeverity] = mapped_column(
        SQLEnum(RuleSeverity, native_enum=False, values_callable=lambda x: [e.value for e in x]),
        nullable=False,
...
class MailboxRuleAlertModel(Base):
    ...
    severity: Mapped[RuleSeverity] = mapped_column(
        SQLEnum(RuleSeverity, native_enum=False, values_callable=lambda x: [e.value for e in x]), nullable=False
    )
