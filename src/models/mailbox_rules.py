...
class MailboxRuleModel(Base):
    ...
    user_email: Mapped[str] = mapped_column(
        ...
        nullable=False,
        index=True,
        comment="Email of the mailbox owner"
    )
    rule_id: Mapped[str] = mapped_column(
        String(255),
        nullable=False,
        comment="Microsoft Graph rule ID"
    )
    rule_name: Mapped[str] = mapped_column(
        String(500),
        nullable=False,
        comment="Display name of the rule"
    )
    rule_type: Mapped[RuleType] = mapped_column(
        SQLEnum(RuleType, name="rule_type_enum"),
        nullable=False,
        comment="Type of mailbox rule"
    )
    is_enabled: Mapped[bool] = mapped_column(
        Boolean,
        default=True,
        nullable=False,
        comment="Whether the rule is enabled in Exchange"
    )
    ...
    has_suspicious_patterns: Mapped[bool] = mapped_column(
        ...
        default=False,
        nullable=False,
        comment="Whether rule contains suspicious patterns"
    )
    ...
    last_scan_at: Mapped[datetime] = mapped_column(
        ...
        default=utc_now,
        nullable=False,
        comment="When rule was last scanned"
    )
...
class MailboxRuleAlertModel(Base):
    ...
    alert_type: Mapped[str] = mapped_column(
        String(50),
        nullable=False,
        comment="Type of alert (forwarding, redirect, etc.)"
    )
    severity: Mapped[RuleSeverity] = mapped_column(
        SQLEnum(RuleSeverity, name="rule_alert_severity_enum"),
        nullable=False
    )
