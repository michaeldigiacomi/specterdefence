...
class Settings(BaseSettings):
    ...
    TRUSTED_PROXIES: list[str] = Field(
        ...
        description="List of trusted proxy IP addresses for secure client IP extraction",
    )
    ...
    ADMIN_PASSWORD_HASH: str = Field(
        default="", description="bcrypt hash of admin password - MUST be set in production"
    )
    ...
    def validate_admin_password_hash(cls, v: str) -> str:
        ...
        if v in default_hashes:
            ...
            raise ValueError(
                ...
                f"Generate a new hash with: {cmd}"
            )
        return v
