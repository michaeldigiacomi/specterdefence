...
// Sync pending alerts
async function syncPendingAlerts(): Promise<void> {
  try {
    const cache = await caches.open(CACHE_NAME);
...
async function syncPendingAcknowledgments(): Promise<void> {
  try {
    const cache = await caches.open(CACHE_NAME);
...
interface ServiceWorkerMessage {
  type: 'SKIP_WAITING' | 'GET_VERSION' | 'CACHE_URLS' | 'CLEAR_CACHE';
  payload: {
    urls?: string[];
  };
}
