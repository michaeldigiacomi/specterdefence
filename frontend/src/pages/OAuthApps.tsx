...
const publisherColors: Record<string, string> = {
  microsoft: 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300 border-blue-100 dark:border-blue-800',
  verified: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300 border-emerald-100 dark:border-emerald-800',
  unverified: 'bg-amber-50 text-amber-700 dark:bg-amber-900/20 dark:text-amber-300 border-amber-100 dark:border-amber-800',
  unknown: 'bg-gray-50 text-gray-700 dark:bg-gray-900/20 dark:text-gray-300 border-gray-100 dark:border-gray-800',
};
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">OAuth Risk Center</h1>
          <p className="mt-2 text-gray-500 dark:text-gray-400 max-w-2xl text-lg font-light">
            Assess and manage third-party integrations. Monitor unverified publishers and high-risk permission grants.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="p-1 bg-gray-100 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 flex">
            <button 
              ...
              className={clsx('p-2 rounded-lg transition-all', viewMode === 'grid' ? 'bg-white dark:bg-gray-700 shadow-sm text-primary-600 dark:text-primary-400' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300')}
            >
              <LayoutGrid className="w-5 h-5" />
            </button>
            <button 
              onClick={() => setViewMode('table')}
              className={clsx('p-2 rounded-lg transition-all', viewMode === 'table' ? 'bg-white dark:bg-gray-700 shadow-sm text-primary-600 dark:text-primary-400' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300')}
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard title="Critical/High Risk" value={highRiskCount} icon={ShieldAlert} color="red" trend={`${((highRiskCount/totalApps)*100).toFixed(0)}% of total`} />
        <StatCard title="Unverified Apps" value={unverifiedCount} icon={AlertTriangle} color="amber" />
      ...
      <div className="space-y-6">
        ...
        {isLoading ? (
          ...
          <div className="py-24 bg-white dark:bg-gray-800 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-700 text-center flex flex-col items-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-2 max-w-xs mx-auto">Try adjusting your filters or search query to find the applications you are looking for.</p>
          </div>
        ) : viewMode === 'grid' ? (
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
                    <th className="text-left px-8 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Application</th>
                    <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Publisher</th>
                    <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Risk Level</th>
                    <th className="text-center px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Access Score</th>
                    <th className="text-right px-8 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
                  {filteredApps.map(app => (
                    <tr key={app.id} className="hover:bg-gray-50/80 dark:hover:bg-gray-700/30 transition-colors group">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <p className="text-sm font-semibold text-gray-900 dark:text-white">{app.display_name}</p>
                              {app.is_new_app && <span className="px-1.5 py-0.5 bg-blue-500 text-white text-[10px] rounded-lg font-bold">NEW</span>}
                            </div>
                            <p className="text-xs text-gray-500 mt-1">{app.consent_count} user consents</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <span className={clsx('px-3 py-1 rounded-full text-xs font-bold border transition-colors', publisherColors[app.publisher_type] || 'bg-gray-100 text-gray-600 border-gray-200')}>
                          {app.publisher_name || app.publisher_type}
                        </span>
                      </td>
                      <td className="px-6 py-5">
                        <span className={clsx('px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest transition-all', riskColors[app.risk_level])}>
                          {app.risk_level}
                        </span>
                      </td>
                      <td className="px-6 py-5">
                            ...
                            <div className="w-full max-w-[60px] bg-gray-100 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                              <div 
                                className={clsx('h-full transition-all duration-1000', 
                                  app.risk_score > 60 ? 'bg-red-500' : app.risk_score > 30 ? 'bg-amber-500' : 'bg-blue-500'
                                )} 
                                style={{ width: `${app.risk_score}%` }} 
                              />
                            </div>
                            <span className="text-sm font-black text-gray-900 dark:text-white">{app.risk_score}</span>
                         </div>
                      </td>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-all group">
      <div className="flex items-start justify-between">
        <div className={clsx('p-3 rounded-2xl border transition-all duration-300 transform group-hover:scale-110', colors[color])}>
          <Icon className="w-6 h-6" />
        </div>
        {trend && <span className="text-[10px] font-black uppercase text-gray-400 tracking-tighter">{trend}</span>}
      </div>
      <div className="mt-5">
        <p className="text-3xl font-black text-gray-900 dark:text-white leading-none tracking-tight">{value}</p>
        <p className="mt-2 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">{title}</p>
      </div>
    </div>
  );
}
    ...
    >
      <div className="flex justify-between items-start mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-extrabold text-gray-900 dark:text-white leading-tight line-clamp-1">{app.display_name}</h3>
            <p className="text-xs font-medium text-gray-500 mt-0.5">{app.publisher_name || 'Internal/Private'}</p>
          </div>
        </div>
        <div className="text-right">
           <div className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">Risk Score</div>
           <div className={clsx('text-xl font-black transition-colors', 
             app.risk_score > 60 ? 'text-red-500' : app.risk_score > 30 ? 'text-amber-500' : 'text-primary-500'
           )}>{app.risk_score}</div>
        </div>
      </div>

      <div className="flex gap-2 mb-6">
         <span className={clsx('px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border transition-all group-hover:border-transparent group-hover:bg-opacity-100', 
           publisherColors[app.publisher_type] || 'bg-gray-100'
         )}>
           {app.publisher_type}
         </span>
         <span className={clsx('px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest', riskColors[app.risk_level])}>
           {app.risk_level}
         </span>
      </div>
      ...
      <div className="mt-8 pt-6 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
        <div className="flex -space-x-2">
          {[...Array(Math.min(3, app.consent_count))].map((_, i) => (
            <div key={i} className="w-7 h-7 rounded-full bg-gray-200 dark:bg-gray-600 border-2 border-white dark:border-gray-800 flex items-center justify-center text-[10px] font-bold text-gray-500">
               <UserCheck className="w-3 h-3" />
            </div>
          ))}
          {app.consent_count > 3 && <div className="text-[10px] font-bold text-gray-400 pl-4">+{app.consent_count - 3} more</div>}
          {app.consent_count === 0 && <span className="text-[10px] font-bold text-gray-400">No user consents</span>}
        </div>
...
function CapIcon({ icon: Icon, active, color }: any) {
  ...
  return (
    <div className={clsx(
      ...
      active ? activeColors[color] : 'bg-gray-50 dark:bg-gray-800 text-gray-300 dark:text-gray-600 border-gray-200 dark:border-gray-700 grayscale'
    )}>
...
function AppDetailsModal({ isOpen, onClose, app }: { isOpen: boolean, onClose: () => void, app: OAuthApp | null }) {
  if (!app) return null;
              ...
              <Dialog.Panel className="relative w-full max-w-4xl rounded-3xl bg-white dark:bg-gray-900 shadow-2xl overflow-hidden">
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex gap-6">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1 font-medium">{app.publisher_name || 'Self-published / Private Application'}</p>
                        <div className="mt-4 flex gap-2">
                          <span className={clsx('px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest', riskColors[app.risk_level])}>
                            {app.risk_level} Risk
                          </span>
                          <span className={clsx('px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest border transition-colors', publisherColors[app.publisher_type] || 'bg-gray-100 text-gray-600')}>
                ...
                <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-2 space-y-8">
                    <section>
                      <h4 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-4">Risk Analysis</h4>
                      <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-6 border border-gray-100 dark:border-gray-700">
                        <div className="flex items-center gap-6 mb-6">
                          <div className="text-center">
                            ...
                            <div className="text-[10px] font-black uppercase text-gray-400 tracking-tighter">Aggregate Score</div>
                          </div>
                          <div className="h-12 w-px bg-gray-200 dark:bg-gray-700" />
                          <p className="text-sm text-gray-600 dark:text-gray-300 font-medium">
                            This application has been flagged based on its permission breadth, publisher reputation, and historical usage patterns in the tenant.
                          </p>
                        </div>
                        
                        <div className="space-y-3">
                          {app.detection_reasons.map((reason, i) => (
                            <div key={i} className="flex gap-3 text-sm font-bold text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-100 dark:border-gray-700">
                    ...
                    <section>
                      <h4 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-4">High Risk Permissions</h4>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {app.high_risk_permissions.map((perm, i) => (
                          <div key={i} className="flex items-center gap-3 p-4 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-2xl">
                             <ShieldAlert className="w-5 h-5 text-red-500" />
                             <span className="text-sm font-black text-red-700 dark:text-red-400">{perm}</span>
                          </div>
                        ))}
                        {app.high_risk_permissions.length === 0 && (
                          <div className="col-span-2 py-8 text-center bg-gray-50 dark:bg-gray-800/50 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-700">
                             ...
                             <p className="text-sm font-bold text-gray-500">No high-risk permissions detected</p>
                          </div>
                        )}
                      </div>
                    </section>
                  ...
                  <div className="space-y-8">
                    <section>
                      <h4 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-4">App Identity</h4>
                      <div className="space-y-4">
                        <MetaItem label="App ID" value={app.app_id} code />
                        <MetaItem label="Tenant ID" value={app.tenant_id} code />
                        <MetaItem label="Publisher ID" value={app.publisher_id || 'Not Available'} />
                        <MetaItem label="Created At" value={app.app_created_at ? new Date(app.app_created_at).toLocaleDateString() : 'Unknown'} />
                      </div>
                    </section>

                    <section>
                      <h4 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-4">Usage Stats</h4>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
                          <div className="text-2xl font-black text-gray-900 dark:text-white">{app.consent_count}</div>
                          <div className="text-[10px] font-black text-gray-400 uppercase mt-1">Total Users</div>
                        </div>
                        <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
                          <div className="text-2xl font-black text-gray-900 dark:text-white">{app.permission_count}</div>
                          <div className="text-[10px] font-black text-gray-400 uppercase mt-1">Total Scopes</div>
                        </div>
                      </div>
                    </section>
                    ...
                    <div className="pt-6">
                       <button 
                        onClick={() => window.open(`https://portal.azure.com/#blade/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/AppDetails/appId/${app.app_id}/activeTab/overview`, '_blank')}
