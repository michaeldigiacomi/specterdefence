...
export default function MFAReport() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const fp: Parameters<typeof apiService.getMFAUsers>[0] = {
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-gray-500 dark:text-gray-400">
            Track MFA enrollment, compliance, and authentication method strength
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">MFA Coverage</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{cov.toFixed(1)}%</p>
            </div>
          </div>
          <div className="mt-3 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
            <div
              className={clsx(
                'h-2 rounded-full',
                cov >= 95 ? 'bg-emerald-500' : cov >= 70 ? 'bg-amber-500' : 'bg-red-500'
              )}
              style={{ width: `${Math.min(cov, 100)}%` }}
            />
          </div>
          <p className="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
            {summary?.mfa_registered_users ?? 0} / {summary?.total_users ?? 0} enrolled
          </p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Admin MFA</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{aCov.toFixed(1)}%</p>
            </div>
          </div>
          <div className="mt-3 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
            <div
              className={clsx('h-2 rounded-full', aCov >= 100 ? 'bg-emerald-500' : 'bg-red-500')}
              style={{ width: `${Math.min(aCov, 100)}%` }}
            />
          </div>
          <p className="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
            {summary?.admins_with_mfa ?? 0} / {summary?.total_admins ?? 0} admins
          </p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex items-center gap-3">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Strong MFA</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {summary?.strong_mfa_users ?? 0}
              </p>
            </div>
          </div>
          <p className="mt-3 text-xs text-gray-500 dark:text-gray-400">
            {summary?.moderate_mfa_users ?? 0} moderate · {summary?.weak_mfa_users ?? 0} weak
          </p>
        </div>
      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-red-800 dark:text-red-300">
                Critical: {adminsWithoutMFA.length} Admin(s) Without MFA
              </h3>
              <div className="mt-3 space-y-2">
                {adminsWithoutMFA.map(a => (
                  <div
                    key={a.id}
                    className="flex items-center gap-3 bg-white dark:bg-gray-800 rounded-lg px-4 py-2.5 border border-red-200 dark:border-red-800"
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {a.user_principal_name} · {a.admin_roles.join(', ')}
                      </p>
                    </div>
                  </div>
                ))}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-900/50">
                ...
                <th className="text-center px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">
                  MFA
                </th>
                <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">
                  Strength
                ...
                users.map(u => (
                  <tr
                    ...
                    className={clsx(
                      ...
                      u.needs_attention && 'bg-red-50/50 dark:bg-red-900/10'
                    )}
                  >
                    <td className="px-6 py-4">
                      ...
                      <p className="text-xs text-gray-500">{u.user_principal_name}</p>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-600 dark:text-gray-300">
                        {u.mfa_methods.length > 0 ? u.mfa_methods.join(', ') : '—'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <span
                        className={clsx(
                          'px-2.5 py-1 rounded-full text-xs font-medium',
                          complianceColors[u.compliance_status]
                        )}
                      >
                        {u.compliance_status.replace('_', ' ')}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      {u.is_admin ? (
                        <span className="px-2 py-0.5 bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-400 text-xs rounded font-medium">
                          Admin
                        </span>
                      ) : (
